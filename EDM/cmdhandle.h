...
{
public:
    CmdHandle(BOOL bVectCmd,QString strCommand,DIGIT_CMD *pDigitCmd,DIGIT_CMD* cmdDefault);
    ~CmdHandle();
    ...
    static BOOL ReadCmdFromFile(QString strPath,QString strFile,vector<QString>* pVector,MAP_ELEC_MAN* pMap);
...
private:
    ...
    int FindAndCalDepth(QString strCmd,DWORD& dwDepth);
    void QStringtoFloat(QString str,float &fFloat);
    int CheckCmdFig(QString strCmd,BOOL *pLaw);
    void AnalyseLastWord(QString &str);
    BOOL HasDot(QString strCmd);

    int GetAntanAngle(BOOL bFourQuant,float fSon,float fMon);

    void VectSort(DIGIT_CMD_VECT* pVectCmd);

    int GetALabel_xDis(int iLabel_A,int iRadius,BOOL bAntiClockWise);
    int GetALabel_zDis(int iLabel_A,int iRadius,BOOL bAntiClockWise);
    int GetCLabel_xDis(int iLabel_C,int iRadius,BOOL bAntiClockWise);
    int GetCLabel_yDis(int iLabel_C,int iRadius,BOOL bAntiClockWise);
};
