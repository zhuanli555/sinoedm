...
void EDM_OP_HOLE::EdmHolePassCtl()
{
	...
	if (m_pEdm->m_stSysSet.stSetNoneLabel.iPoleMode==0)
		return;

	if (m_pEdm->m_stSysSet.stSetNoneLabel.iPoleMode==1)
	{
		EdmHolePassCtl_1();
		return;
	}

	if (m_pEdm->m_stSysSet.stSetNoneLabel.iPoleMode==3)
	{
		EdmHolePassCtl_3();
		return;
	}
...
void EDM_OP_HOLE::EdmHolePassCtl_1()
{
	float fStable;
	int iCnt=0;
	int iCalc =0;
	...
	int iJudgePt;
	int iPulseSleep;
	BOOL bPoleStable = FALSE;
	float fToff;
	int iToffIndex;
	int iOpLabelWorkPos;
	static int iSleep = 0;

	iStartStable = ((float)m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecOral.iOpLenAll)*0.4;
	iStartStable = min(iStartStable,15000);

	iJudgePt = ((float)m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecOral.iOpLenAll)*0.6;
	...
	m_stOpCtrl.stZeroCtrl.stPassCtl.iBackStable[m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex] = m_stMacPassPara.iBackCnt;
	m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStable[m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex] = m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltage;
	if (m_stOpCtrl.stZeroCtrl.stPassCtl.bEntryOver)
		iSleep++;

	if (m_stOpCtrl.stZeroCtrl.stPassCtl.enPassMode == PASS_NONE
		&& iOpLabelWorkPos>=m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecPage[0].iOpLen+1000
		&& !m_stOpCtrl.stZeroCtrl.stPassCtl.bPoleStable
		&& m_stOpCtrl.stZeroCtrl.stPassCtl.iStageCnt>1
		&& iSleep>=3)
	{
		iCalc = m_stOpCtrl.stZeroCtrl.stPassCtl.iSum/m_stOpCtrl.stZeroCtrl.stPassCtl.iStageCnt*0.7;
		m_stOpCtrl.stZeroCtrl.stPassCtl.iFeedStd = iCalc;
	...
	if (iOpLabelWorkPos >= m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecOral.iJudgePos
		&& m_stOpCtrl.stZeroCtrl.stPassCtl.bEntryOver && m_stOpCtrl.stZeroCtrl.stPassCtl.iStageCnt>3)
	{
		/////////////////////
		m_stOpCtrl.stZeroCtrl.stPassCtl.bPoleStable = TRUE;
		iPulseSleep = m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecPage[m_stOpStatus.stCycle.iOpPage].iToff;
		if (!m_stOpCtrl.stZeroCtrl.stPassCtl.bStop)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.bStop = TRUE;			 
            GetElecToff(iPulseSleep,&iToffIndex);
			iToffIndex += m_pEdm->m_stSysSet.stSetNoneLabel.iToff;
		...
		m_stOpCtrl.stZeroCtrl.stPassCtl.bPole = TRUE;
...
void EDM_OP_HOLE::EdmHolePassCtl_3()
{
	int iCnt=0;
	int iCalc =0;
	int iStartStable;
	...
	m_stOpCtrl.stZeroCtrl.stPassCtl.iBackStable[m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex] = m_stMacPassPara.iBackCnt;
	m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStable[m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex] = m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltage;
	m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStable[m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex] = m_stMacPassPara.iFeedCnt-m_stMacPassPara.iBackCnt;
	m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRec[m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex] = m_stMacPassPara.iFeedCnt-m_stMacPassPara.iBackCnt;
	m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRec[m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRecIndex] = m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltage;

	iCnt = 0;
	for (int i=0;i<=m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex&& m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex>0;i++)
	{
		iCnt += m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStable[i];
	}
	if (m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex>0)
	{
		m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedCalc = iCnt/(m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex+1);
	}
		...
		&& m_stOpCtrl.stZeroCtrl.stPassCtl.bEntryOver && m_stOpCtrl.stZeroCtrl.stPassCtl.iStageCnt>3)
	{
		/////////////////////
		m_stOpCtrl.stZeroCtrl.stPassCtl.bPoleStable = TRUE;
		iPulseSleep = iPulseSleep = m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecPage[m_stOpStatus.stCycle.iOpPage].iToff;
		if (!m_stOpCtrl.stZeroCtrl.stPassCtl.bStop)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.bStop = TRUE;			 
            GetElecToff(iPulseSleep,&iToffIndex);
			...
			iCalc = 0;
			for (int i=0;i<m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex;i++)
			{
				iCalc += m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRec[i];
			}
			m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStd = m_stPassSet.fSpeed * iCalc/(m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex+1);
			m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStd = (m_stPassSet.fElec* (((float)iPulseSleep)/fToff)+0.17)
				*((float)m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageSum/(float)m_stOpCtrl.stZeroCtrl.stPassCtl.iStageCnt);

			m_stOpStatus.stCycle.stPassChart.bSet = TRUE;
			m_stOpStatus.stCycle.stPassChart.iSpeedFactor = (float)m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStd/m_stPassSet.fSpeed;
			m_stOpStatus.stCycle.stPassChart.fElecFactor = (float)m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStd/m_stPassSet.fElec;
			m_stOpStatus.stCycle.stPassChart.iPosJudge = m_pOpFile->m_mpElecMan[m_pOpFile->m_strElec].stElecOral.iJudgePos;
			m_stOpStatus.stCycle.stPassChart.iElecJudge = m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStd;
			m_stOpStatus.stCycle.stPassChart.iSpeedJudge = m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStd;
		}

		m_stOpCtrl.stZeroCtrl.stPassCtl.bPole = TRUE;
		if (m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedCalc< m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStd 
			&& m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex>0 
			&& m_stOpCtrl.stZeroCtrl.stPassCtl.enPassMode == PASS_NONE)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.enPassMode = PASS_LOW;
		}

		iCnt = 0;
		if (m_stOpCtrl.stZeroCtrl.stPassCtl.enPassMode == PASS_NONE && m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex>0)
		{
			if (m_stOpCtrl.stZeroCtrl.stPassCtl.iPosMax<=iOpLabelWorkPos)
			{
				for (int i=0;i<=m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex;i++)
				{
					iCnt += m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStable[i];	
				}

				iCnt =  iCnt/(m_stOpCtrl.stZeroCtrl.stPassCtl.iStabelIndex+1);
				if (iCnt<=m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageStd )
				{
					m_stOpCtrl.stZeroCtrl.stPassCtl.enPassMode = PASS_HIGH;
				}
			}			
		}
	}
	...
	if (++m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex>m_stPassSet.iSpeedFilterCnt-1)
	{
		m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex = m_stPassSet.iSpeedFilterCnt-1;
		for (int i=0;i<m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedIndex;i++)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStable[i]= m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedStable[i+1];
		}
	}

	if (++m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRecIndex>m_stPassSet.iElecFilterCnt-1)
	{
		m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRecIndex  = m_stPassSet.iElecFilterCnt-1;
		for (int i=0;i<m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRecIndex;i++)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRec[i]= m_stOpCtrl.stZeroCtrl.stPassCtl.iVoltageRec[i+1];
		}
	}

	if (++m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex >= 49)
	{
		m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex = 49;
		for (int i=0;i<m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRecIndex;i++)
		{
			m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRec[i] = m_stOpCtrl.stZeroCtrl.stPassCtl.iSpeedRec[i+1];
		}
	}
}
